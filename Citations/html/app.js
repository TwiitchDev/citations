...
$(function () {
    ...
    function SuggestionClick(el){
        ...
        for(i = 0; i < classes.length / 2; i++){
            document.getElementById(classes[i*2]).value = classes[i*2+1]
        }
    }
    ...
    document.getElementById("sus_firstname").oninput = function(){
        ...
        $.post('http://'+resourcename+'/getpersons', JSON.stringify({
            ...
            for(i = 0; i < persons.length; i++){
                base = '<div class="sus_firstname '+persons[i].firstname+' sus_lastname '+persons[i].lastname+'"><h1>'+persons[i].name+'</h1></div>';
        ...
        $.post('http://'+resourcename+'/getviolations', JSON.stringify({
            ...
            if(violations != null && violations.code != null) {
                ...
                for(i = 0; i < violations.length; i++){
                    base = '<div class="vio_code_1 '+violations[i].code+' vio_title_1 '+violations[i].title+'"><h1>'+violations[i].name+'</h1></div>';
                    $("#suggest_scroll").append(base)
                }
                suggestionHosts = scrollEL.getElementsByTagName("div")
                for(i = 0; i < suggestionHosts.length; i++){
    ...
    document.getElementById("vio_code_2").oninput = function(){
        ...
        $.post('http://'+resourcename+'/getviolations', JSON.stringify({
            ...
            if(violations != null && violations.code != null) {
                ...
                scrollEL.innerHTML = ""
                for(i = 0; i < violations.length; i++){
                    base = '<div class="vio_code_2 '+violations[i].code+' vio_title_2 '+violations[i].title+'"><h1>'+violations[i].name+'</h1></div>';
                    $("#suggest_scroll").append(base)
                }
                suggestionHosts = scrollEL.getElementsByTagName("div")
                for(i = 0; i < suggestionHosts.length; i++){
    ...
    document.getElementById("vio_code_3").oninput = function(){
        ...
        $.post('http://'+resourcename+'/getviolations', JSON.stringify({
            ...
            if(violations != null && violations.code != null) {
                ...
                scrollEL.innerHTML = ""
                for(i = 0; i < violations.length; i++){
                    base = '<div class="vio_code_3 '+violations[i].code+' vio_title_3 '+violations[i].title+'"><h1>'+violations[i].name+'</h1></div>';
                    $("#suggest_scroll").append(base)
                }
                suggestionHosts = scrollEL.getElementsByTagName("div")
                for(i = 0; i < suggestionHosts.length; i++){
