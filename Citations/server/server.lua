...
RNE("updatecitationinfo", function(info)
    src = source
    Player = QB.Functions.GetPlayer(src)
    slots = exports[Config.inventoryResource]:GetSlotsByItem(Player.PlayerData.items, "citation")
    for i = 1, #slots, 1 do
        item = exports[Config.inventoryResource]:GetItemBySlot(src, slots[i])
    ...
    exports['oxmysql']:execute("SELECT * FROM `rd_citations` WHERE `citationid` = @citationid", {
        ...
        if result[1] then
            ...
            if Player.PlayerData.money[paymethod] >= totalPrice then
                ...
                {0, citationid})
                exports["qb-management"]:AddMoney(result[1].society, totalPrice)
